...
<!-- Íàñòðîéêà ñòèëÿ -->
<xsl:variable name="nomer_jur"><!--ïîêàçûâàòü ñèìâî ¹ ïåðåä íîìåðîì æóðíàëà (1 - äà; 0 - íåò)--><xsl:text>0</xsl:text></xsl:variable>
<xsl:variable name="i_title"><!-- íàçâàíèå êóðñèâîì (1 - äà; 0 - íåò)--><xsl:text>0</xsl:text></xsl:variable>
<!-- *** -->	
  ...
  <xsl:template name="templ_str_PagesCountinousShort" >
	...
	<!--Åñëè ÿçûê ðóññêèé-->
	<xsl:choose>
		<xsl:when test="$_LCID=1049 or $_LCID=1058">
		<!-- ðóññêèé -->
			<xsl:text>C.</xsl:text>
		</xsl:when>
  ...
  <xsl:template name="templ_prop_GroupSeparator" ><!-- Ðàçäåëèòåëü ìåæäó èñòî÷íèêàìè â ñêîáêàõ [1, 4, 6], èçìåíèë â òåêñòå, çäåñü îñòàâèë êàê åñòü!!! -->
    <xsl:param name="LCID" />
    <xsl:variable name="_LCID">
      <xsl:call-template name="localLCID">
        <xsl:with-param name="LCID" select="$LCID"/>
  ...
  <xsl:template name="templ_prop_EnumSeparator" > <!-- Ðàçäåëèòåëü! -->
    <xsl:param name="LCID" />
    <xsl:variable name="_LCID">
      <xsl:call-template name="localLCID">
        <xsl:with-param name="LCID" select="$LCID"/>
							...
							<xsl:choose>
								...
								<xsl:otherwise>
									...
									<xsl:for-each select="msxsl:node-set($ListPopulatedWithMain)/b:Citation/b:Source/b:Author/b:Main/b:NameList/b:Person">
										<xsl:choose>
											<xsl:when test="position() >= 2 and $cAuthors = 2">	
													<xsl:text>, </xsl:text>
		                  ...
		                  <xsl:if test="string-length($pages)>0">
		                    <xsl:choose>
		                      ...
		                      <xsl:when test="string-length($pages)=string-length(translate($pages, $prop_APA_Hyphens, ''))">
		                        <!--<xsl:call-template name="templ_str_PageShort"/>--><xsl:text>, c.</xsl:text>
		                      </xsl:when>
		                      <xsl:otherwise>
		                        <xsl:call-template name="templ_str_PagesCountinousShort"/>
		                      </xsl:otherwise>
...
<!-- Ðàçäåëèòåëü ìåæäó èñòî÷íèêàìè â ñêîáêàõ [1, 4, 6],--><xsl:text>, </xsl:text>
							<!--<xsl:call-template name="templ_prop_GroupSeparator"/>-->
						</xsl:if>
					...
					<body>
						...
						<xsl:for-each select="msxsl:node-set($SortedList)/b:Bibliography/b:Source">
							<xsl:element name="p">
								...
								<xsl:variable name="volVolume"> <!-- Òîì -->								
									<xsl:if test="string-length($volume) > 0">
										<xsl:call-template name="StringFormat">
											<xsl:with-param name="format">
											  <xsl:choose>
								...
								<xsl:variable name ="Pages_b">
	<!--Îïðåäåëÿþ ÿçûê--><xsl:variable name="_LCID"><xsl:call-template name="localLCID"><xsl:with-param name="LCID" select="$LCID"/></xsl:call-template></xsl:variable>
				                  <xsl:if test="string-length($pages)>0">
						<xsl:if test="$_LCID=1049 or $_LCID=1058 or $_LCID=1033">
				                    <xsl:value-of select="$pages"/>
						    <xsl:call-template name="templ_prop_Space"/>
						</xsl:if>
				                    <xsl:choose>
						...
						<xsl:if test="$_LCID!=1049 and $_LCID!=1058 and $_LCID!=1033">
				                    <xsl:call-template name="templ_prop_Space"/>
				                    <xsl:value-of select="$pages"/>
						</xsl:if>
				                  </xsl:if>
									...
									<xsl:call-template name ="formatHyphenSection">
										<xsl:with-param name ="FirstElement">
											<xsl:call-template name ="formatColonElement">
												...
												<xsl:with-param name ="SecondParam" select ="$volVolume"/>
												<xsl:with-param name ="ThirdParam" select ="b:NumberVolumes"/>
												<xsl:with-param name ="FourthParam" select ="$Pages_b"/>
											</xsl:call-template>
										</xsl:with-param>
									</xsl:call-template>
								</xsl:variable>
                            ...
                            <xsl:call-template name="templ_prop_GroupSeparator"/>													
								...
								<xsl:variable name ="IssueVolumeHyphenSection">
									<xsl:call-template name ="formatHyphenSection">
										<xsl:with-param name ="FirstElement">
											<xsl:choose><!--Åñëè åñòü òîì-->
												<xsl:when test="string-length($volVolume)>0">
													<xsl:choose>
														<xsl:when test="string-length(b:Issue)>0">
															<xsl:value-of select="$volVolume"/>, <xsl:if test="$nomer_jur=1">&#8470;</xsl:if><xsl:value-of select="b:Issue"/>
														</xsl:when>
														<xsl:otherwise>
															<xsl:value-of select="$volVolume"/>
														</xsl:otherwise>
												...
												<xsl:otherwise><!--Åñëè åñòü òîìà íåò-->
													<xsl:if test="$nomer_jur=1">&#8470;</xsl:if><xsl:value-of select="b:Issue"/>
											<!--<xsl:call-template name ="formatColonElement">
								...
								<xsl:variable name ="PagesHyphenSection_b">
									<xsl:call-template name ="formatHyphenSection">
										<xsl:with-param name ="FirstElement" select ="$Pages_b"/>
									</xsl:call-template>
								</xsl:variable>
								...
								<!-- ÍÓÌÅÐÀÖÈß Â ÑÏÈÑÊÅ -->
								<xsl:value-of select="b:RefOrder"/>.<!--<b>-->&#160;<xsl:value-of select="$MainAuthors"/><!--</b>-->
								<xsl:call-template name="templ_prop_Space" />
								<xsl:if test="string-length(b:TitlePrefix) > 0">
									<xsl:value-of select="b:TitlePrefix"/>
								</xsl:if>
									<xsl:choose><!--Íàêëîííîå íàçâàíèå-->
										<xsl:when test="$i_title=1">
								...
								<xsl:variable name ="SourceType"> <!-- [êíèãà], [æóðíàë] è ò.ï.-->
                ...
                <xsl:if test = "b:SourceType='ElectronicSource' and string-length(b:PublicationTitle) >0">
                  ...
                  <xsl:value-of select="b:PublicationTitle" />
								...
								<xsl:if test="b:SourceType='Report'">
									...
									<xsl:value-of select ="$CityPublisherYearHyphenSection"/>
									<xsl:value-of select ="$PagesHyphenSection_b"/>
								</xsl:if>
		...
		<xsl:choose>
			<xsl:when test="./b:SourceType='Book'">
				<xsl:choose>
					...
					<!--<xsl:when test="string-length(./b:Author/b:Editor)>0">Editor</xsl:when>    ðåäàêòîð â ñàìîì íà÷àëå-->
...
<xsl:template name="formatPersons">
	...
	<xsl:template name ="formatColonElement">
		...
		<xsl:if test ="string-length($SecondParam)>0" >
